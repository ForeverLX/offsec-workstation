// StatusMonitor Module - System stats popup
import QtQuick
...
PanelWindow {
    ...
    function parseMeminfo(text) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line.indexOf("MemTotal:") === 0) {
                total = parseInt(line.replace(/\D/g, "")) || 1
            } else if (line.indexOf("MemAvailable:") === 0) {
                available = parseInt(line.replace(/\D/g, "")) || 0
            }
        }
        if (total > 0) {
            ramPercent = 100 * (total - available) / total
        }
    }
    ...
    function parseCpuStat(text) {
        ...
        if (cpuInitialized && prevCpuStats.length === values.length) {
            ...
            if (deltaTotal > 0) {
                cpuPercent = Math.round(100 * (deltaTotal - deltaIdle) / deltaTotal)
            }
        } else {
            cpuInitialized = true
        }
    ...
    function parseTemp(text) {
        ...
        if (!isNaN(val)) {
            tempCelsius = Math.round(val / 1000)
        }
    }

    function parseDisk(text) {
        var txt = text.trim()
        if (txt !== "") diskUsage = txt
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: parseCpuStat(text)
        }
    }
    ...
    Process {
        ...
        running: true
        stdout: StdioCollector {
            onStreamFinished: parseMeminfo(text)
        }
    }
    ...
    Process {
        ...
        running: true
        stdout: StdioCollector {
            onStreamFinished: parseTemp(text)
        }
    }
    ...
    Process {
        ...
        running: true
        stdout: StdioCollector {
            onStreamFinished: parseDisk(text)
        }
    }
    ...
    Process {
        ...
        running: true
        stdout: StdioCollector {
            onStreamFinished: parseUptime(text)
        }
    }

    Timer {
        interval: 3000
        running: statusMonitor.visible
        repeat: true
        onTriggered: {
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                Text {
                    text: "RAM: " + Math.round(ramPercent) + "%"
                    color: mocha.text
                    font.pixelSize: 11
                }
